...
(
	state=$(grep '"call" : "de.junit4.Callee4#innerMethod",\|state' results/$version/de.test.junit4.Callee4Test_onlyCallMethod1.js | grep "innerMethod" -A 1 | grep '"state" : "SLOWER",' | grep -o 'SLOWER')
	if [ "$state" != "SLOWER" ]
	then
		echo "State for de.junit4.Callee4#innerMethod in de.test.junit4.Callee4Test#onlyCallMethod1.html has not the expected value SLOWER, but was $state!"
		cat results/$version/de.test.junit4.Callee4Test_onlyCallMethod1.js
		exit 1
	else
...
(
	sourceMethodLine=$(grep "de.junit4.Callee4.method1_" results/$version/de.test.junit4.Callee4Test_onlyCallMethod1.js -A 3 | head -n 3 | grep innerMethod)
